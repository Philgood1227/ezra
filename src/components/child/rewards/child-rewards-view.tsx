...
export function ChildRewardsView({
  ...
  const [spentToday, setSpentToday] = React.useState(() =>
    Math.max(0, Math.trunc(initialSpentToday)),
  );
  ...
  React.useEffect(() => {
    setSpentToday(Math.max(0, Math.trunc(initialSpentToday)));
  }, [initialSpentToday]);
  ...
  const currentStars = Math.max(0, Math.max(0, initialStars) - spentToday);
  const isRewardFutureLocked = React.useCallback(
    (reward: RewardCardModel): boolean => reward.cost > currentStars + 10,
    [currentStars],
  );

  const availableRewards = React.useMemo(() => {
    return rewardModels.filter((reward) => !isRewardFutureLocked(reward) && reward.cost <= currentStars);
  }, [currentStars, isRewardFutureLocked, rewardModels]);
  ...
  const futureRewards = React.useMemo(() => {
    return rewardModels.filter((reward) => isRewardFutureLocked(reward));
  }, [isRewardFutureLocked, rewardModels]);
  ...
  function confirmClaimReward(): void {
    ...
    startClaimTransition(async () => {
      ...
      setSpentToday(Math.max(0, Math.trunc(actionData.spentToday)));
      ...
      <div className="mx-auto w-full max-w-[1080px] space-y-6">
        ...
        <section className="space-y-4">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-xl font-black text-slate-900">Recompenses futures</h2>
          </div>
          ...
          {futureRewards.length > 0 ? (
            <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
              {futureRewards.map((reward, index) => (
                <motion.article
                  ...
                  whileHover={{ scale: 1.03 }}
                  className="rounded-2xl border-4 border-slate-300 bg-slate-100 p-4 text-center opacity-70"
                >
                  <div className="text-5xl grayscale">{reward.emoji}</div>
                  <p className="mt-2 text-sm font-black text-slate-700">{reward.title}</p>
                  <div className="mt-2 inline-flex items-center gap-1 rounded-full border-2 border-slate-300 bg-white px-2 py-1 text-xs font-black text-slate-700">
                    <StarIcon className="h-3.5 w-3.5 fill-slate-500 text-slate-500" />
                    {reward.cost}
                  </div>
                </motion.article>
              ))}
            ...
            <div className="rounded-2xl border-4 border-slate-200 bg-slate-50 p-4 text-center text-sm font-black text-slate-600">
              Tout est deja a ta portee aujourd&apos;hui, bravo !
            </div>
          )}
        </section>
